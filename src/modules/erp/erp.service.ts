...
export class ErpService {
  ...
  async publishVideo(teacherId: number, dto: PublishVideoDto) {
    ...
    if (lesson.teacherId !== teacherId) {
      throw new ForbiddenException(
        'You can only upload videos for your own lessons',
      );
    }
  ...
  async updateHomeworkPolicy(
    teacherId: number,
    homeworkId: number,
    dto: UpdateHomeworkPolicyDto,
  ) {
    ...
    if (dto.maxAttempts) {
      ...
      if ((maxAttemptsUsed._max.attemptNo || 0) > dto.maxAttempts) {
        throw new BadRequestException(
          'maxAttempts cannot be less than already used attempts',
        );
      }
  ...
  async submitHomework(studentId: number, dto: SubmitHomeworkDto) {
    ...
    if (!isMember) {
      throw new ForbiddenException(
        'Student is not a member of this lesson group',
      );
    }
    ...
    if (attemptsUsed >= homework.maxAttempts) {
      throw new ConflictException(
        'Submission attempt limit reached for this homework',
      );
    }
    ...
    if (isLate && !homework.allowLateSubmission) {
      throw new ForbiddenException(
        'Deadline has passed and late submission is disabled',
      );
    }
  ...
  async reviewHomework(teacherId: number, dto: ReviewHomeworkDto) {
    ...
    if (!latestResponse) {
      throw new NotFoundException(
        'No submission found to review for this student',
      );
    }
  ...
  async getTeacherDashboard(teacherId: number) {
    const [groups, lessons, homeworks] = await Promise.all([
    ...
    const upcomingDeadlines = await this.prisma.homework.findMany({
      where: {
        teacherId,
        deadlineAt: {
          gte: new Date(),
    ...
    const submissionsToReview = await this.prisma.homeworkResponse.count({
      where: {
        homework: {
          teacherId,
        },
  ...
  async getStudentDashboard(studentId: number) {
    ...
    const submissionMap = new Map<
      number,
      { status: HomeworkStatusStudent; attemptNo: number }
    >();
    ...
    return assigned.map((homework) => ({
      ...homework,
      submitStatus: submissionMap.get(homework.id)?.status || 'NOT_SUBMITTED',
      attemptNo: submissionMap.get(homework.id)?.attemptNo || 0,
    }));
  }
  ...
  async getStudentProgress(studentId: number) {
    ...
    const approved = grades.filter(
      (g) => g.status === HomeworkStatus.APPROVED,
    ).length;
    const rejected = grades.filter(
      (g) => g.status === HomeworkStatus.REJECTED,
    ).length;
    const pending = grades.filter(
      (g) => g.status === HomeworkStatus.PENDING,
    ).length;
    ...
    const onTime = responses.filter(
      (r) => r.status === HomeworkStatusStudent.COMPLETED,
    ).length;
    const late = responses.filter(
      (r) => r.status === HomeworkStatusStudent.DELAY,
    ).length;
  ...
  async getFinanceReport() {
    ...
    const grouped = new Map<
      number,
      { courseName: string; amount: Prisma.Decimal }
    >();
  ...
  async getSuperAdminAnalytics() {
    const [teacherCount, studentCount, groupCount, responseStats] =
      await Promise.all([
  ...
  private async ensureTeacherOwnsHomework(
    teacherId: number,
    homeworkId: number,
  ) {
    const homework = await this.prisma.homework.findUnique({
    ...
    if (homework.teacherId !== teacherId) {
      throw new ForbiddenException(
        'Teacher can only manage own homework tasks',
      );
    }
